...
def main():
    ...
    dataset_path = "results/data/prediction"
    ...
    pred_data = torch.load(
        os.path.join(dataset_path, f"pred_data_cycle_{CYCLE_NUM}.pt"),
        weights_only=False,
    )
    calibrator_path = os.path.join(evaluation_path, f"calibrator_cycle_{CYCLE_NUM}.pt")
